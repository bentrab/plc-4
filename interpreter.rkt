...
(define interpret-function
  (lambda (statement environment return break continue throw)
    (cond
      ((null? (body statement)) environment) ;checks if the function body is empty
      (else (insert (func-name statement) (body statement) environment))))) ;checks if there are any parameters



; The same as interpret-statement-list except at the end it returns the environment
(define interpret-funclist
  (lambda (statement-list environment return break continue throw)
    (cond 
        ((null? statement-list) (pop-frame environment)) 
        (else (interpret-funclist (cdr statement-list) (interpret-statement (car statement-list) environment return break continue throw) return break continue throw)))))

; evealuates a function call and returns its associated environment
(define interpret-func-env
 (lambda (statement-list environment return break continue throw)
  (cond
    ...
    (else (if (list? (call/cc
                  ...
                  (interpret-func-env (cdr statement-list) (call/cc
                                                                              (lambda (breakreturn)
                                                                                (interpret-statement (car statement-list) environment breakreturn break continue throw)))
                                                        return break continue throw)
                  environment)))))
...
(define add-params-env
  (lambda (param-names param-values environment throw)
    (cond
      ...
      ((list? param-names) (add-params-env (params param-names) (params param-values) (insert (car param-names) (eval-expression (car param-values) (pop-frame environment) throw) environment) throw))
